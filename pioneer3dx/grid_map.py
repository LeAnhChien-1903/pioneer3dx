...
from pioneer3dx.bresenham import *

THRESHOLD_P_FREE = 0.3
THRESHOLD_P_OCC = 0.6
...
class GridMap:
    ...
    def calculate_MLE(self):
        ...
        for x in range(self.l.shape[0]):
            for y in range(self.l.shape[1]):
                # cell is tree
                if self.l[x][y] < log_odds(THRESHOLD_P_FREE):
                    self.l[x][y] = log_odds(0.01)
                # cell is occupied
                elif self.l[x][y] > log_odds(THRESHOLD_P_OCC):
    ...
    def find_neighbors(self, x, y):
        """
            Find neighboring pixels to pixel (x, y)
        """
        X_neighbors = []
        Y_neighbors = []
        ...
        return zip(X_neighbors, Y_neighbors)
